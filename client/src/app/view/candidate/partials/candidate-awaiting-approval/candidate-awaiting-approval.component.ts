...
@Component({
  selector: 'app-candidate-awaiting-approval',
  templateUrl: './candidate-awaiting-approval.component.html',
  styleUrls: ['./candidate-awaiting-approval.component.scss']
})
export class CandidateAwaitingApprovalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._route.queryParams.subscribe(data => {
      ...
      this.getOpportunitiesAwaiting(start, end);
    });
  }
   ...
   * @return {Promise<void>}
   */
  public async getOpportunitiesAwaiting(dateStart: string, dateEnd: string, limit: string = '50'): Promise<void> {
    ...
    if (startDate > endDate) {
      ...
      try {
        ...
        const response = await this._candidateService.getOpportunitiesAwaiting(data, this.pagination);
        ...
        if (response.pagination.total_count === this.awaitingOpportunities.length) {
          this.loadMoreCheck = false;
        }
        else if (response.pagination.total_count !== this.awaitingOpportunities.length) {
